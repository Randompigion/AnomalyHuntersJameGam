...
func _update_chunks(current_chunk: Vector2i) -> void:
	...
	for x in range(-render_radius, render_radius + 1):
		for y in range(-render_radius, render_radius + 1):
			needed_coords.append(current_chunk + Vector2i(x, y))
			
	for coord in needed_coords:
		if not active_chunks.has(coord) and not coord in pending_chunks:
	...
	for coord in active_chunks.keys():
		if abs(coord.x - current_chunk.x) > unload_margin or abs(coord.y - current_chunk.y) > unload_margin:
...
static func generate_chunk_image(chunk_coord: Vector2i, chunk_size: int, master_seed: int, options: Dictionary = {}) -> Image:
	...
	for y in range(sh):
		...
		for x in range(sw):
			...
			img.set_pixel(x, y, Color(clamp((r + dk) / 255.0, 0.0, 1.0), clamp((g + dk) / 255.0, 0.0, 1.0), clamp((b + dk) / 255.0, 0.0, 1.0), 1.0))
	...
	for i in range(ns1):
		...
		img.set_pixel(sx, sy, Color(v/255.0, v/255.0, v/255.0, 1.0))
		...
		for dy in range(-(pr + 2), pr + 3):
			for dx in range(-(pr + 2), pr + 3):
				...
				if dist < pr:
					...
					img.set_pixel(nx, ny, Color(v_rgb.x/255.0, v_rgb.y/255.0, v_rgb.z/255.0, 1.0))
